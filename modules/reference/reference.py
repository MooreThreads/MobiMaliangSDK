...
from shared import SDModel, MTGPU_DETECTION, device
            ...
            if MODE == "write":
                if gn_auto_machine_weight >= self.gn_weight:
                    var, mean = torch.var_mean(
                        x.to("cpu"), dim=(2, 3), keepdim=True, correction=0
                    )
                    var = var.to(device)
                    mean = mean.to(device)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                ref_xt = self.scheduler.add_noise(
                    ref_image_latents, noise, t.reshape(1)
                )
...
def reference2img(
    ...
    reference2img_pipeline = StableDiffusionReferencePipeline(
        vae=sd_model.vae,
    ...
    if text_inversion_model_name_or_path is not None:
        ...
        for text_inversion_weight in text_inversion_model_name_or_path:
            reference2img_pipeline.load_textual_inversion(text_inversion_weight)

    generator = None
    if seed is not None:
        generator = torch.Generator(sd_model.device).manual_seed(seed)

    results = reference2img_pipeline(
        prompt=prompt,
        ref_image=ref_image,
        height=height,
        width=width,
    ...
    if (
        ...
        reference2img_pipeline.enable_model_cpu_offload()
