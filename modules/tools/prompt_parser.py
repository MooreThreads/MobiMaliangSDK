...
# get names and weights of loras from a prompt in the format of <lora:name:weight>
def get_lora_from_prompt(prompt):
    ...
    while res is not None:
        lora = res.group()
        idx = prompt.find(lora)
        final_prompt += prompt[:idx]
        prompt = prompt[idx + len(lora) :]
        lora = lora.split(":")
        try:
            lora_name = lora[1]
            lora_weight = float(lora[2][:-1])
        except:
            pass
        else:
            result.append([lora_name, lora_weight])
...
# whether a prompt contains chinese or not
def contain_zh(s):
    for w in s:
        if "\u4e00" <= w <= "\u9fff":
            return True
