...
from modules.shared import MTGPU_DETECTION
...
class DeepDanbooruWrapper:
    def __init__(
        ...
        self.include_ranks = include_ranks

    def load_models(self, model_path):

        self.model = DeepDanbooruModel()
        state_dict = torch.load(model_path)
        self.model.load_state_dict(state_dict, map_location="cpu")
    ...
    def tag_multi(self, pil_image, force_disable_ranks=False):
        ...
        image_np = np.expand_dims(np.array(pil_image, dtype=np.float32), 0) / 255
        image_tensor = torch.from_numpy(image_np).to(torch.float32).to(self.device)
        with torch.no_grad():
            y = self.model(image_tensor)[0].detach().cpu().numpy()
        probability_dict = {}
